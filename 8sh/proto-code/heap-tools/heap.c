...
dump1:
   ...
   printf("size:   %04x (%d bytes %s)\n", 
                  *(unsigned short *)&heap_vars[curr+4], 
                  *(unsigned short *)&heap_vars[curr+4] & 0x7fff, 
                 (*(unsigned short *)&heap_vars[curr+4] & 0x8000)>0?"used":"free");
   if (*(unsigned short *)&heap_vars[curr+4]>0) 
   {
      printf("name:   %s (len %ld)\n", &heap_vars[curr+6], strlen(&heap_vars[curr+6])+1);
      printf("value:  %s (len %ld)\n", 
                      &heap_vars[curr+6+strlen(&heap_vars[curr+6])+1], 
                      strlen(&heap_vars[curr+6+strlen(&heap_vars[curr+6])+1])+1);
   }
...
unsigned short heap_var_exists(char *line_in)
{
   unsigned short var_ptr = 2;
   unsigned char k;
   unsigned short var_start = 0;
   unsigned short var_end = strlen(line_in)-1;
